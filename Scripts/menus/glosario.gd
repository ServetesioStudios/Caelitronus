...
func _ready():
	...
	if GameManager.player_data != null:
		...
		$Panel/General/ScrollContainerMaldiciones/Maldiciones/Reves/Button.set_block_signals(true)
...
func ocultar(nombre):
	var personajes = $Panel/General/ScrollContainerPersonajes/Personajes
	
	# El nodo con el nombre del personaje ES el fondo
	var fondo = personajes.get_node(nombre)
	
	# El TextureRect es el personaje
	var personaje = personajes.get_node(nombre + "/TextureRect")
	
	# Oscurecer el fondo
	fondo.modulate = Color(0.35, 0.35, 0.35, 1)
	
	# Personaje completamente negro
	personaje.modulate = Color(0, 0, 0, 1)
	
	# Mostrar ???
	personajes.get_node(nombre + "/Label").text = "???"
	
	# Desactivar botón
	personajes.get_node(nombre + "/Button").disabled = true
	personajes.get_node(nombre + "/Button").set_block_signals(true)
...
func _on_descripcion_pressed() -> void:
	$Panel/Detalle/Descripcion.disabled = true
	$Panel/Detalle/Descripcion.set_block_signals(true)
	
	$Panel/Detalle/Religion.disabled = false
	$Panel/Detalle/Religion.set_block_signals(false)
	
	$Panel/Detalle/Alma.disabled = false
	$Panel/Detalle/Alma.set_block_signals(false)
...
func _on_religion_pressed() -> void:
	...
	$Panel/Detalle/Religion.set_block_signals(true)
	
	$Panel/Detalle/Descripcion.disabled = false
	$Panel/Detalle/Descripcion.set_block_signals(false)
...
func _on_alma_pressed() -> void:
	...
	$Panel/Detalle/Alma.set_block_signals(true)
